...
public class DefinitionLoaders {
	...
	/*test*/ static void parseLang(Document doc, Locator locator, URL url, boolean addon) throws Exception {
		...
		for (Iterator<Element> it = root.getElements("system-property").iterator(); it.hasNext();) {
		...
		for (Iterator<Element> it = root.getElements("javascript").iterator(); it.hasNext();) {
			final Element el = (Element) it.next();
		...
		for (Iterator<Element> it = root.getElements("javascript-module").iterator(); it.hasNext();) {
			final Element el = (Element) it.next();
			langdef.addJavaScriptModule(IDOMs.getRequiredAttributeValue(el, "name"),
					IDOMs.getRequiredAttributeValue(el, "version"));
		}

		for (Iterator<Element> it = root.getElements("stylesheet").iterator(); it.hasNext();) {
			final Element el = (Element) it.next();
		...
		for (Iterator<Element> it = root.getElements("zscript").iterator(); it.hasNext();) {
			final Element el = (Element) it.next();
			final String zslang;
		...
		for (Iterator<Element> it = root.getElements("component").iterator(); it.hasNext();) {
			...
			for (Iterator<Element> i = el.getElements("mold").iterator(); i.hasNext();) {
			...
			for (final Map.Entry<String, String> me: parseCustAttrs(el).entrySet()) {
				compdef.addCustomAttribute((String) me.getKey(), (String) me.getValue());
			}

			for (final Map.Entry<String, String> me: parseProps(el).entrySet()) {
	...
	private static void parsePI(LanguageDefinition langdef, Document doc) throws Exception {
		for (Iterator<Item> it = doc.getChildren().iterator(); it.hasNext();) {
